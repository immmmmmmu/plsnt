...
スキル（Pleasanter 操作の知見集）は **どちらの方法でも `plsnt init` で導入**できます（バイナリに同梱）。
...
スキルはリポジトリには同梱されていません。`plsnt init` で導入してください（上記「[スキルの導入](#スキルの導入plsnt-init)」を参照）。
...
> **注意**: ソース同梱の業務サンプル（`scripts/*.sh`）を実行するには、Windows では **Git Bash**（Git for Windows に付属）
> または **WSL** が必要です。
